...
class StudentSubjectsScoreDetailsAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        """Answer."""
        # Making Query: "StudentSubjectsScore."
        personel = Personnel.objects.get(id=student_id)
        student_subjects_score = StudentSubjectsScore.objects

        # Subjects1: "Math"
        sj1_credit = [] # credit
        sj1_score = []  # score
        sss_sj1 = student_subjects_score.filter(student_id=student_id, subjects_id=1)
        for field in sss_sj1:
            sj1_credit.append(field.credit)
            sj1_score.append(field.score)

        # Subjects2: "Physics"
        sj2_credit = [] # credit
        sj2_score = [] # score
        sss_sj2 = student_subjects_score.filter(student_id=student_id, subjects_id=2)
        for field in sss_sj2:
            sj2_credit.append(field.credit)
            sj2_score.append(field.score)

        # Subjects3: "Chemistry"
        sj3_credit = [] # credit
        sj3_score = []  # score
        sss_sj3 = student_subjects_score.filter(student_id=student_id, subjects_id=3)
        for field in sss_sj3:
            sj3_credit.append(field.credit)
            sj3_score.append(field.score)

        # Subjects4: "Algorithm"
        sj4_credit = [] # credit
        sj4_score = []  # score
        sss_sj4 = student_subjects_score.filter(student_id=student_id, subjects_id=4)
        for field in sss_sj4:
            sj4_credit.append(field.credit)
            sj4_score.append(field.score)

        # Subjects5: "Coding"
        sj5_credit = [] # credit
        sj5_score = []  # score
        sss_sj5 = student_subjects_score.filter(student_id=student_id, subjects_id=5)
        for field in sss_sj5:
            sj5_credit.append(field.credit)
            sj5_score.append(field.score)

        # Grade validation
        avg_subjects_score = [((sj1_score[0]+sj2_score[0]+sj3_score[0]+sj4_score[0]+sj5_score[0])/5)]
        subjects_score_list = [sj1_score[0], sj2_score[0], sj3_score[0], sj4_score[0], sj5_score[0], avg_subjects_score[0]]      
        subjects_grade_list = []
        for subject_score in subjects_score_list:
            if subject_score >= 80 and subject_score <= 100:
                subjects_grade_list.append("A")
            elif subject_score >= 75 and subject_score < 80:
                subjects_grade_list.append("B+")
            elif subject_score >= 70 and subject_score < 75:
                subjects_grade_list.append("B")
            elif subject_score >= 65 and subject_score <70:
                subjects_grade_list.append("C+")
            elif subject_score >= 60 and subject_score < 65:
                subjects_grade_list.append("C")
            elif subject_score >= 55 and subject_score < 60:
                subjects_grade_list.append("D+")
            elif subject_score >= 50 and subject_score < 55:
                subjects_grade_list.append("D")
            else:
                subjects_grade_list.append("F")
        
        data_context ={
            "student":{
                "id": personel.id,
                "full_name": personel.first_name+' '+personel.last_name, 
                "school": str(personel.school_class.school.title) 
            },
            "subject_detail": [
                {
                    "subject": str(student_subjects_score.get(id=1).subjects.title),
                    "credit": sj1_credit[0],
                    "score": sj1_score[0],
                    "grade": subjects_grade_list[0],
                },
                {
                    "subject": str(student_subjects_score.get(id=2).subjects.title),
                    "credit": sj2_credit[0],
                    "score": sj2_score[0],
                    "grade": subjects_grade_list[1],
                },
                {
                    "subject": str(student_subjects_score.get(id=3).subjects.title),
                    "credit": sj3_credit[0],
                    "score": sj3_score[0],
                    "grade": subjects_grade_list[2],
                },
                {
                    "subject": str(student_subjects_score.get(id=4).subjects.title),
                    "credit": sj4_credit[0],
                    "score": sj4_score[0],
                    "grade": subjects_grade_list[3],
                },
                {
                    "subject": str(student_subjects_score.get(id=5).subjects.title),
                    "credit": sj5_credit[0],
                    "score": sj5_score[0],
                    "grade": subjects_grade_list[4],
                },
            ],

            "grade_point_average": subjects_grade_list[5],
        }
        return Response(data_context, status=status.HTTP_200_OK)
...
class PersonnelDetailsAPIView(APIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        """Answer."""
        personel = Personnel.objects.filter()
        
        personel_data_pattern = {
...
class SchoolStructureAPIView(APIView):
    ...
    def get(request, *args, **kwargs):
        ...
        ss_obj = school_structure

        ss_objs_head_title_list = []
        ss_objs_junior_title_list = []
        ss_objs_high_title_list = []
        ss_objs_sub_mor1_list = []
        ss_objs_sub_mor2_list = []
        ss_objs_sub_mor3_list = []
        ss_objs_sub_mor4_list = []
        ss_objs_sub_mor5_list = []
        ss_objs_sub_mor6_list = []

        # Looping for adding to list.
        for obj in ss_obj:
            # Head มัธยมต้น, มัธยมปลาย
            if obj.title == "มัธยมต้น":
                ss_objs_head_title_list.append(obj.title)
            if obj.title == "มัธยมปลาย":
                ss_objs_head_title_list.append(obj.title)
            # ม.ต้น
            if obj.title in ['ม.1', 'ม.2', 'ม.3']: # Title
                ss_objs_junior_title_list.append(obj.title)
                
            if obj.parent == 2:
                ss_objs_sub_mor1_list.append(obj.title)
            if obj.parent == 10:
                ss_objs_sub_mor2_list.append(obj.title)
            if obj.parent == 18:
                ss_objs_sub_mor3_list.append(obj.title)

            # ม.ปลาย
            if obj.title in ['ม.4', 'ม.5', 'ม.6']: # Title
                ss_objs_high_title_list.append(obj.title)
             
            if obj.parent == 27:
                ss_objs_sub_mor4_list.append(obj.title)
            if obj.parent == 35:
                ss_objs_sub_mor5_list.append(obj.title)
            if obj.parent == 43:
                ss_objs_sub_mor6_list.append(obj.title)
